...
class dd: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        pageControl.translatesAutoresizingMaskIntoConstraints = false
        scrollView.contentSize = CGSize(width: scrollView.frame.width * CGFloat(labels.count), height: scrollView.frame.height)
        ...
        pageView.frame.size = CGSize(width: scrollView.frame.width/2, height: scrollView.frame.height/2)
        pageView.center = CGPoint(x: scrollView.frame.width * CGFloat(index) + scrollView.center.x, y: scrollView.center.y)
